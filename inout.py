...
def write_to_file(dest, x, y, data, starttime, endtime, plot=False):
    """
    As the name suggests writes the data to file. If plot=True, the data will also be plotted
    """

    print("Opening file %s%dto%d.dat for writing"%(dest, starttime/1000, endtime/1000))
    with open("%s%dto%d.dat"%(dest, starttime/1000, endtime/1000), "w") as f:
    ...
    if(plot):
        plotdata(
                "%s%dto%d.dat"%(dest, starttime/1000, endtime/1000),
                "Order parameter between %d to %d ns"%(starttime/1000, endtime/1000)
                )
